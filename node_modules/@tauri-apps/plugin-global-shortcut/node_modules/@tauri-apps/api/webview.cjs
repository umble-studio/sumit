...
class Webview {
    ...
    async onFileDropEvent(handler) {
        const unlistenFileDrop = await this.listen(event.TauriEvent.FILE_DROP, (event) => {
            handler({
                ...event,
                payload: {
                    type: 'drop',
        ...
        const unlistenFileHover = await this.listen(event.TauriEvent.FILE_DROP_HOVER, (event) => {
            handler({
                ...event,
                payload: {
                    type: 'hover',
        ...
        const unlistenCancel = await this.listen(event.TauriEvent.FILE_DROP_CANCELLED, (event) => {
